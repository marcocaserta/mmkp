...
int ** s_best_brkga;		//!< best assignment found by brkga
int ** y_best_brkga;		//!< best assignment found by brkga
...
void compute_statistics(int ** y, int ** s, int & infeasNeigh, int & infeasNat, int & infeasGender, int & infeasWG);
/************************ FUNCTIONS ******************************/
...
int main(int argc, char *argv[])
{
   ...
   // create neighborhoods (large and small) <---- REMOVE THIS
   ...
   s_best_brkga = new (int*[nPeriods]);
   y_best_brkga = new (int*[nPeriods]);
   for (int t = 0; t < nPeriods; t++)
     {
       s_best_brkga[t] = new int[nSeats];
       y_best_brkga[t] = new int[nStudents];
     }

   BRKGA< SampleDecoder, MTRand > algorithm(n, p, pe, pm, rhoe, decoder, rng, K, MAXT, R, x, N_set, counting, countingReduced);
	
   unsigned generation = 0;	// current generation
   const unsigned X_INTVL = 100; // how often to exchange best individuals
   const unsigned X_NUMBER = 2;	// exchange top 2 best
   const unsigned MAX_GENS = 250;	// run for MAX_GENS generations
   ...
   infeasGender = 0;
   infeasWG     = 0;
   print_chart(s_best_brkga);
   compute_statistics(y_best_brkga, s_best_brkga, infeasNeigh, infeasNat, infeasGender, infeasWG); 

   cout << " ====================================" << endl;
   cout << "| Summary Statistics bRKGA Solution |" << endl;
   cout << " ====================================" << endl;
...
void get_chart(IloModel model, IloCplex cplex, ThreeD x_ilo)
{
   ...
   compute_statistics(y, s, infeasNeigh, infeasNat, infeasGender, infeasWG);
...
/// Compute statistics of the solution held in s and y
void compute_statistics(int ** y, int ** s, int & infeasNeigh, int & infeasNat, int & infeasGender, int & infeasWG)
{
