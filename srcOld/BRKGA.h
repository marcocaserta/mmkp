...
BRKGA(unsigned n, unsigned p, double pe, double pm, double rhoe,
      const Decoder& refDecoder, RNG& refRNG, unsigned K = 1, unsigned MAX_THREADS = 1, RInside & R = NULL, int ** x = NULL, std::vector < std::vector < int> > & N_set = NULL, int ** counting = NULL, int ** countingReduced = NULL);
...
template< class Decoder, class RNG >
  BRKGA< Decoder, RNG >::BRKGA(unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe, const Decoder& decoder, RNG& rng, unsigned _K, unsigned MAX, RInside & _R, int ** _x, std::vector < std::vector < int> > & _N_set, int ** _counting, int ** _countingReduced) : n(_n), p(_p), pe(unsigned(_pe * p)), pm(unsigned(_pm * p)), rhoe(_rhoe), refRNG(rng), refDecoder(decoder), K(_K), MAX_THREADS(MAX), previous(K, 0), current(K, 0),  R(_R), x(_x), N_set(_N_set), counting(_counting), countingReduced(_countingReduced) {
...
template< class Decoder, class RNG >
  ...
  current[i]->setFitness(j, refDecoder.decode((*current[i])(j), R, x, N_set, counting, countingReduced) );
	}
...
template< class Decoder, class RNG >
   inline void BRKGA< Decoder, RNG >::evolution(Population& curr, Population& next) {
	...
	for(int i = int(pe); i < int(p); ++i) {
	  next.setFitness( i, refDecoder.decode(next.population[i], R, x, N_set, counting, countingReduced) );
	}
